defmodule JidoWatch.Adapter.TraktInMemoryTest do
  ...
  defp setup_for(:refresh_valid),
    do: TraktInMemory.start!(refresh_chain: %{"ref-good" => @tokens})
  ...
  defp setup_for(:watched_movies_valid),
    do: TraktInMemory.start!(watched_movies: [%{"plays" => 1}])

  defp setup_for(:watched_movies_unauthorized),
    do: TraktInMemory.start!(unauthorized_access_tokens: ["tok-dead"])

  defp setup_for(:stats_valid),
    do: TraktInMemory.start!(stats: %{"episodes" => %{"watched" => 1}})
