...
Adapter: Trakt.HTTP (src: lib/jido_watch/trakt/http.ex; integration: test/adapter/trakt_http_test.exs)
  ...
  recent_watches/2
    ...
    when Trakt returns a representative /sync/history payload
      then each entry preserves type, watched_at, and the matching movie or episode+show object with title and imdb id
  ...
  watched_shows/2
    when given a valid access token
      then it GETs /sync/watched/shows with extended=full, bearer auth, trakt-api-version and trakt-api-key headers
        and returns the parsed list of shows with play counts and genres
    when Trakt returns a representative /sync/watched/shows payload with extended=full
      then each entry preserves plays and the show object's title and genres
    if Trakt responds with 401
      then the error is :unauthorized
    if Trakt responds 200 with a body that is not a list
      then the error wraps the status and body
  ...
  watched_movies/2
    when given a valid access token
      then it GETs /sync/watched/movies with extended=full, bearer auth, trakt-api-version and trakt-api-key headers
        and returns the parsed list of movies with play counts and genres
    when Trakt returns a representative /sync/watched/movies payload with extended=full
      then each entry preserves plays and the movie object's title and genres
    if Trakt responds with 401
      then the error is :unauthorized
    if Trakt responds 200 with a body that is not a list
      then the error wraps the status and body
